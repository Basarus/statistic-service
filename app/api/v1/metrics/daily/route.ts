...
export async function GET(req: Request) {
  try {
    ...
    const result = await db().query(
      `
      SELECT *
      FROM stats.agg_daily_metrics m
      WHERE ($1::date IS NULL OR m.metric_date >= $1::date)
        AND ($2::date IS NULL OR m.metric_date <= $2::date)
    ...
    return fail('Failed to load daily metrics', 500, (error as Error).message);
  }
}
