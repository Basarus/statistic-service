...
export class StatsAggregateService {
  ...
  async runRawToDailyAggregation(): Promise<void> {
    ...
    const result = await this.dataSource.query(
      ...
      SELECT
        MAX(source_events.occurred_at) AS max_occurred_at,
        MAX(source_events.id)::text AS max_event_id,
        COUNT(source_events.id)::int AS processed_count
      FROM source_events
      `,
  ...
  async runDailyToMonthlyAggregation(): Promise<void> {
    ...
    for (const item of changedMonths) {
      await this.dataSource.query(
        `
        DELETE FROM stat_aggregate_monthly
